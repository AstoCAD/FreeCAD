...
public:
    ...
    void setArcGeometry(const QPointF& center, const QPointF& anchor)
    {
        QPointF radial = anchor - center;
        const double radialLength =
            std::hypot(radial.x(), radial.y());
        ...
        const QPointF radius = radial * radiusLength;
...
void TaskSectionView::updateSectionControls()
{
    ...
    for (size_t i = 1; i < bounds.size(); ++i) {
        ...
        for (size_t kind = 0; kind < kinds.size(); ++kind) {
            ...
            std::function<void()> callback;
            if (kind == 0) {
                callback = [this, midpoint, towardStart]() {
                    startSingleOffset(midpoint, towardStart);
                };
            }
            else if (kind == 1) {
            ...
            if (kinds[kind] == SectionActionGlyph::Kind::Arc) {
                item->setArcGeometry(sceneCenter, scenePosition);
            }
            else {
...
public:
    ...
    void mouseMoveEvent(QMouseEvent* event) override
    {
        ...
        if (m_mode == Mode::SeekCenter) {
            ...
            m_task->setInteractiveParameters(toSectionOrigin(m_center), toBaseDirection(m_direction));
        }
        else {
            ...
            m_direction = Base::Vector3d(std::cos(radians), std::sin(radians), 0.0);
            updatePreview(m_center, m_direction);
            m_task->setInteractiveParameters(toSectionOrigin(m_center), toBaseDirection(m_direction));
        }
        event->accept();
    ...
    void mousePressEvent(QMouseEvent* event) override
    {
        ...
        if (m_mode == Mode::SeekCenter) {
            ...
            m_preview = nullptr;
            m_task->updateTemporarySectionLine(m_center, m_direction);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TaskSectionView::startSingleOffset(double along, bool towardStart)
{
    ...
    m_pendingMinimumAlong = -m_temporaryHalfLength;
    m_pendingMaximumAlong = m_temporaryHalfLength;
    const QPointF tangent(-m_displayedDirection.y, -m_displayedDirection.x);
    const double startOffset = projectOffsetStart(
        m_temporaryCenter + tangent * along).second;
    beginSingleOffset(along, startOffset, towardStart);
    showUnappliedPreview(false);
                                     ...
                                     bool towardStart)
{
    ...
    m_sectionControlsEnabled = false;
    hideSectionHandles();
    Q_UNUSED(minimumAlong);
    Q_UNUSED(maximumAlong);
    m_pendingArc = {along, 0.0, towardStart};
    m_hasPendingArc = true;
    showUnappliedPreview(false);
    m_graphicsView->activateHandler(
        new ArcOffsetHandler(this, towardStart));
}
    ...
    if (QPushButton* applyButton = buttonBox->button(QDialogButtonBox::Apply)) {
        auto updateApplyButton = [applyButton](bool valid) {
            ...
            applyButton->setToolTip(
                valid ? QObject::tr("Rebuild display now. May be slow for complex models.")
