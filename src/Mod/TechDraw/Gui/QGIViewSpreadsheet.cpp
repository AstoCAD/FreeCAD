...
public:
    ...
    void setCellValues(const QString& displayValue, const QString& editValue)
    {
...
void QGIViewSpreadsheet::createEditor()
{
    ...
    connect(m_table,
            ...
            [this](int row, int column) {
                ...
                if (item) {
                    Q_EMIT cellChanged(row, column, item->data(Qt::EditRole).toString());
                }
            });
...
void QGIViewSpreadsheet::updateCellItem(int tableRow, int tableColumn)
{
    ...
    item->setCellValues(displayValue, editValue);
}
