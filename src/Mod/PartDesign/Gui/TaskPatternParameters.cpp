...
{
    apply(parametersWidget);
    apply(parametersWidget2);
}

void TaskPatternParameters::apply(PartGui::PatternParametersWidget* pwidget)
{
    auto pattern = getObject();
    if (!pattern || !pwidget) {
        return;
    ...
    FCMD_OBJ_CMD(pattern, "Reversed = " << pwidget->getReverse());
    FCMD_OBJ_CMD(pattern, "Mode = " << pwidget->getMode());
    pwidget->applyQuantitySpinboxes();

    FCMD_OBJ_CMD(pattern, pwidget->getSpacingPatternsAsString());
}
