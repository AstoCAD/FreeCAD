...
using DSHSymbolController =
    DrawSketchDefaultWidgetController<DrawSketchHandlerSymbol,
...
template<>
void DSHSymbolController::doChangeDrawSketchHandlerMode()
{
    switch (handler->state()) {
        case SelectMode::SeekFirst: {
            auto& firstParam = onViewParameters[OnViewParameter::First];
...
std::list<Gui::InputHint> DrawSketchHandlerSymbol::lookupSymbolHints(int method, int state)
{
    ...
    auto it = std::find_if(SymbolHintTable.begin(),
                           SymbolHintTable.end(),
